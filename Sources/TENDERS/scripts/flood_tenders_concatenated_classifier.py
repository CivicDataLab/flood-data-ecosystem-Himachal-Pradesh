...
data_path = os.getcwd() + r'\\HP\\flood-data-ecosystem-Himachal-Pradesh\\Sources\\TENDERS\\data\\hp_tenders_23-24\\'
...
for csv in csvs:
    ...
    POSITIVE_KEYWORDS = ['flood', 'landslide', 'flash flood', 'heavy rain', 'embankment', 'Drainage',' River', 'Khad', 'Bridge', 'Nallah', 'Nalla', 'Lake', 'Relief', 'Erosion','Silt', 'Culvert', 'Sluice', 'Dyke', 'Storm water drain', 'Immediate', 'dam', 'irrigation', 'rain', 'pond', 'recharge', 'bank', 'restoration', 'rain damages', 'rain shelter', 'hydrology', 'breast wall', 'drain', ' retaining wall', 'pot holes', 'cross drainage', ' ambulance road', 'road', 'rainwater harvesting system']
    global NEGATIVE_KEYWORDS
    NEGATIVE_KEYWORDS = ['Flood Lights', 'pipe', 'pipes', 'supply', 'beautification', 'installation']
    ...
    for index, row in tenders_df.iterrows():
        ...
        if 1 <= published_date.month <= 2:
            monsoon = "Winter"
        elif 3 <= published_date.month <= 5:
                monsoon = "Pre-Monsoon"
    ...
    scheme_kw = {#'ridf','sdrf','sopd','cidf','ltif',
                 'LWSS', 'NDRF', 'SDMF', 'Flow Irrigation Scheme', 'WSS', 'Water supply scheme', 'R-RWBCIS', 'NCAP funds' }
    ...
    ROADS_BRIDGES_EMBANKMENTS_KEYWORDS = ['roads', 'bridges', 'road', 'bridge', 'storm water drain' ,'drain', 'box culvert', 'culvert' ,'embankment', 'dyke', 'silt', 'sluice']
    for index, row in tenders_df.iterrows():
        tender_slug = str(row['tender_externalreference']) + ' ' + str(row['tender_title']) + ' ' + str(row['Work Description'])
        tender_slug = re.sub(r'[^a-zA-Z0-9 \n\.]', ' ', tender_slug)
    ...
    IMMEDIATE_MEASURES_KEYWORDS = ['relief', 'package','immediate', 'emergency'
#        'sdrf','im','i/m','gr','g/r','relief','package','pkt','immediate', 'emergency', 'pk', 'g.r.', 'i.m.'
]
    REPAIR_RESTORATION_IMPROVEMENTS_KEYWORDS = [#'improvement', 'imp.', 'impvt', 'impt.', 'repair',
                                                ...
                                                'improvement', 'imp', 'repair', 'repairing', 'restoration', 'reconstruction','raising', 'strengthening', 'r/s', 'upgradation', 'renovation', 'up-gradation', 'widening', 'extension','replacement', 'check dam', 'construction', 'bridge', 'retrofitting', 'drain']
    ...
    PREPAREDNESS_KEYWORDS = [#'shelter', 'shelters', 'tarpaulin', 'shelter ',
                             ...
                             'shelter', 'rain shelter', 'periodical maintainance','water proofing', 'maintenance', 'annual maintenance', 'protection', 'scoured', 'recharge', 'cleaning ','Flood Protection work']
    for index, row in tenders_df.iterrows():
    ...
    tenders_df.to_csv(os.getcwd()+r'/HP/flood-data-ecosystem-Himachal-Pradesh/Sources/TENDERS/data/hp_tenders_23-24/flood_tenders/'+filename,
                            encoding='utf-8',
                            index=False)
                            
    
# Add explanation for this piece of code
data_path = os.getcwd() + r'/flood-data-ecosystem-Himachal-Pradesh/Sources/TENDERS/data/hp_tenders_23-24'
