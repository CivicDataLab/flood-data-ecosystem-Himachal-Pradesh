...
HP_SDS = gpd.read_file(os.getcwd()+r'\Maps\hp_tehsil_final.geojson', driver='GeoJSON')

#RC_HQs = list(HP_SDS[HP_SDS.HQ=='y']['revenue_ci'])

idea_frm_tenders_df  = pd.read_csv(os.getcwd()+r'/Sources/TENDERS/data/floodtenders_districtgeotagged.csv', keep_default_na=False)

MASTER_DFs = []
for FOCUS_DISTRICT in tqdm(HP_VILLAGES.dtname.unique()):
    ...
    for index,row in HP_VILLAGES[HP_VILLAGES.dtname==FOCUS_DISTRICT].iterrows():
        if row["VILNAM_SOI"]:
...
MASTER_DF.to_csv(os.getcwd()+r'/Sources/TENDERS/data/floodtenders_Tehsil-geotagged.csv')