...
def drop_columns(df, protected=None):
    """
    Drop any column that is entirely NaN or entirely zero,
    except for any columns in `protected`.
    """
...
def main():
    ...
    # For all disaster events, uncomment the following lines:
    #hc_master = hc
    ...
    education = pd.read_csv(root + r'/Sources/HPSDMA/data/losses-and-damages/Raw Data/extracted_tables/dbo.app_education_loss_main.csv')
    education_details = pd.read_csv(root + r'/Sources/HPSDMA/data/losses-and-damages/Loss Data/keys_identifiers/higher_education_loss_details.csv')
    education = education.loc[education['status'] == 'Active']
    ...
    print(education_merged.columns)    
    protected = {'schools_damaged'}
    education_cleaned = drop_columns(education_merged, protected=protected)
    ...
    education_block = education_block.merge(education_details[['education_loss_det_id','education_loss_id','disaster_id','school_name','damage_details','status','schooldayslost','classroomsdamaged','amountloss',]], on='education_loss_id',how='left')

    ungeo_df = education_block
    # Load datasets
    ...
    ungeo_df = geotag_subdistrict(ungeo_df, villages_df, threshold=0.5, conflict_margin=0.05, narrow_by_district=True)
    ...
    output_path = root + r"/Sources/HPSDMA/data/losses-and-damages/Loss Data/flood_losses/education_flood_geocoded.csv"  # <-- Update with desired output file path
