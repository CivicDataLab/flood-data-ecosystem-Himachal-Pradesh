...
def geotag_subdistrict(ungeo_df, villages_df, threshold=0.80, conflict_margin=0.05, narrow_by_district=True):
    ...
    for idx, row in tqdm(ungeo_df.iterrows(), total=ungeo_df.shape[0], desc="Geotagging Subdistrict"):
        ...
        fallback_parts = []
        for col in ['block_name', 'circle_name', 'division_name', 'location','ulb_name']:
            val = row.get(col, '')
            if pd.notnull(val) and str(val).strip():
                fallback_parts.append(str(val))
...
def main():
    ...
    power_merged = power_master.merge(disaster_events[['disaster_id','incidentdate']], on="disaster_id", how="left")
    ...
    power_circle = power_cleaned.merge(circles[['CId','circle']], left_on='circle_id',right_on='CId',how='left')
    ...
    output_path = r"D:\CivicDataLab_IDS-DRR\IDS-DRR_Github\HP\flood-data-ecosystem-Himachal-Pradesh\Sources\HPSDMA\data\losses-and-damages\Loss Data\all_disaster_losses\power_losses_geocoded.csv"  # <-- Update with desired output file path
