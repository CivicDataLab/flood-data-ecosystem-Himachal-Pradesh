...
def main():
    ...
    structure = pd.read_csv(root + r'/Sources\HPSDMA\data\losses-and-damages\Loss Data\master_losses\Structure_Loss.csv')
    structure_details = pd.read_csv(root + r'/Sources\HPSDMA\data\losses-and-damages\Loss Data\keys_identifiers\Structure-typemaster.csv')
    ...
    structure_dist = structure_cleaned.merge(district[['district_name','district_id']], on='district_id',how='left')
    ...
    ungeo_df = geotag_subdistrict(ungeo_df, villages_df, threshold=0.2, conflict_margin=0.05, narrow_by_district=True)
    ...
    output_path = root + r'\Sources\HPSDMA\data\losses-and-damages\Loss Data\flood_losses\structure_flood_geocoded.csv'  # <-- Update with desired output file path
