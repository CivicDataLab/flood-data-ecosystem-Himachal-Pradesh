...
def geotag_tehsil_by_name(ungeo_df, tehsil_shp_path,
    ...
    tehsil_gdf['tehsil_clean'] = tehsil_gdf[tehsil_name_field].apply(clean_text)

    candidates = tehsil_gdf['tehsil_clean'].tolist()
    mapping   = dict(zip(tehsil_gdf['tehsil_clean'], tehsil_gdf[tehsil_name_field]))
    ...
    for _, row in ungeo_df.iterrows():
        ...
        best = ""    # default if nothing matches
        alt  = ""    # default alternative
            ...
            if matches:
                ...
                if score >= threshold:
                    ...
                    if len(matches) > 1 and (matches[0][1] - matches[1][1]) < conflict_margin:
                        alt = mapping[matches[1][0]]
        # now append **every iteration**, regardless of whether txt was empty
        tehs_best.append(best)
        tehs_alt.append(alt)

    # sanity check
    assert len(tehs_best) == len(ungeo_df) == len(tehs_alt)

    ungeo_df['tehsil_best']        = tehs_best
    ungeo_df['tehsil_alternative'] = tehs_alt
    return ungeo_df
...
def main():
    ...
    hort_merged = hort_master.merge(flood_events[['disaster_id','incidentdate']], on="disaster_id", how="left")
    ...
    output_path = r"D:\CivicDataLab_IDS-DRR\IDS-DRR_Github\HP\flood-data-ecosystem-Himachal-Pradesh\Sources\HPSDMA\data\losses-and-damages\Loss Data\flood_losses\horticulture_flood_geocoded.csv"  # <-- Update with desired output file path
